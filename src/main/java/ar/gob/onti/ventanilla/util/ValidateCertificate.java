...
public class ValidateCertificate {
	...
	/**
	 * Valida un certificado digital (Expiración, CRL y CA permitida en ese
	 * orden )
	 * @param certificate
		...
		try { 
			if (this.isValidExpirationDate(certificate)) {
				...
				if (!salida.equals("")) {
					...
					System.out.println("La validación de la CRL fue exitosa");
				}
			} else { 
	...
	private String validateCRL(Certificate certificate) throws KeyStoreException, CRLException {
		...
		try {
			...
			salida = "EXCEPCION SGI: Se produjo una excepción del tipo CertificateException"; 
			System.out.println(salida);
			e.printStackTrace();
			return salida;
		} catch (IOException e) {
			salida = "EXCEPCION SGI: Se produjo una excepción del tipo IOException";
			System.out.println(salida);
			e.printStackTrace();
			return salida;
		}
